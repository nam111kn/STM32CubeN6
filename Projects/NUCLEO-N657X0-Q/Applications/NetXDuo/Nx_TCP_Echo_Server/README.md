...
Next, this program can be run in boot from flash mode. This can be done by following the instructions below:
 ...
 - Resort to CubeProgrammer to add a header to the generated binary Nx_TCP_Echo_Server.bin with the following command
   - *STM32_SigningTool_CLI.exe -bin Nx_TCP_Echo_Server.bin -nk -of 0x80000000 -t fsbl -o Nx_TCP_Echo_Server-trusted.bin -hv 2.3 -dump Nx_TCP_Echo_Server-trusted.bin*
   - The resulting binary is Nx_TCP_Echo_Server-trusted.bin.
 - Next, in resorting again to CubeProgrammer, load the binary and its header (Nx_TCP_Echo_Server-trusted.bin) in Nucleo board external Flash at address 0x7000'0000.
