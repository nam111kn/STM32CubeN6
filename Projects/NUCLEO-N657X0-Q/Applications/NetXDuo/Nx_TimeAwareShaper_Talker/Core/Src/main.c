...
static void MX_ETH_Init(void)
{
  ...
  */
  HAL_ETH_PTP_GetConfig(&heth, &ptpconfig);

  ptpconfig.Timestamp = ENABLE;
  ptpconfig.TimestampInitialize = ENABLE;
  ...
  ptpconfig.TimestampAddend = (ULONG)((0x100000000UL * PTP_REF_CLK) / ptp_clk);
...
void HAL_GPIO_EXTI_Falling_Callback(uint16_t GPIO_Pin)
{
  ...
  {
    tx_semaphore_put(&TAS_Semaphore);
  }
}
/* USER CODE END 4 */
